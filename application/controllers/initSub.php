				...
				if(!$check) {
					...
					$result1 = $this->arts_model->getProjID($this->input->post('ActTitle'), $this->input->post('OrgName'));
						 if($result1){
									 ...
									 if($result2){
									 		foreach ($result2 as $row)
									 		{
									 			...
									 			$this->db->insert('astatus', $stat);
									 				...
									 				echo '<div class="success">'.'Your submission has been successfully created'. '<br>'. '<a href="dts_Cont" class="btn btn-default" >'.'Proceed to DTS'.'</a>'. '<a href="initSub" class="btn btn-default" >'.'Submit Again'.'</a>'.'</div>';
									 			
									 		}
									 }
								 	else //else getSubID
									   {
									  	echo '<div class="success">'.'Submission Failed'. '<br>'.  '<br>'. '<a href="initSub" class="btn btn-default" >'.'Submit Again'.'</a>'.'</div>';
									   }
							   ...
							   {
							  	echo '<div class="success">'.'Submission Failed'. '<br>'. 'Error'. '<br>'.  '<br>'. '<a href="initSub" class="btn btn-default" >'.'Submit Again'.'</a>'.'</div>';
							   }
					}
			 	else // else checkTitle
				   {
				  	echo '<div class="success">'.'Submission Failed'. '<br>'. 'The Activity has an Initial Submission already'.  '<br>'. '<a href="initSub" class="btn btn-default" >'.'Submit Again'.'</a>'.'</div>';
				   }
			   }
